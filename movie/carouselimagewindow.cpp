    ...
    , m_currentDrawImageIndx(0)
{
    ...
    this->setObjectName("movieWidget");
}
...
void CarouselImageWindow::initChangeImageButton()
{
    ...
    for (int i = 0; i < m_imageList.count(); i++)
    {
        ...
        pButton->setFixedSize(QSize(10, 10));
        pButton->setCheckable(true);
//        pButton->setStyleSheet("QPushButton{border-image:url(:/Resources/select1.png);border-radius:1px;}\
...
//    if (!file.exists()){
//    QMessageBox::information(NULL,"测试","文件存在");
//    }
    m_imageList.append(QPixmap(imageFileName));
}
...
void CarouselImageWindow::startPlay()
{
    ...
    else if (m_imageList.count() > 1)
    {
        ...
        m_currentPixmap =m_imageList.at(m_currentDrawImageIndx);
...
void CarouselImageWindow::onImageChangeTimeout()
{
    ...
    if (m_currentDrawImageIndx >= m_imageList.count())
    {
        m_currentDrawImageIndx = 0;
    }
    //m_nextPixmap = QPixmap(m_imageFileNameList.at(m_currentDrawImageIndx));
    m_nextPixmap = QPixmap(m_imageList.at(m_currentDrawImageIndx));
...
//    else if (m_imageFileNameList.count() == 1)
    else if (m_imageList.count() == 1)
    {
        ...
        QPixmap backPixmap = m_imageList.first();
        painter.drawPixmap(imageRect, backPixmap.scaled(imageRect.size()));
...
void CarouselImageWindow::onImageSwitchButtonClicked(int buttonId)
{
    ...
    if (m_currentDrawImageIndx == -1)
    {
        ...
        m_currentDrawImageIndx = m_imageList.count() - 1;
    }
